create or replace function public.replace_avatar(p_new_path text default null, p_expected_path text default null)
...
begin
  ...
  if p_new_path is not null and p_new_path !~ ('^'||uid::text||'/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.jpg$') then raise exception using errcode='22023',message='Invalid avatar path'; end if;
  ...
  if (p_new_path is not null and current_path is not distinct from p_new_path)
    or (p_new_path is null and p_expected_path is not null and current_path is null) then
    return jsonb_build_object('outcome','already_updated','current_path',current_path);
  end if;
  ...
  if current_path is not null and current_path is distinct from p_new_path then insert into public.avatar_cleanup_queue(storage_path,user_id) values(current_path,uid) on conflict do nothing; end if;
