create index avatar_cleanup_queue_user_created_idx on public.avatar_cleanup_queue(user_id,created_at);
...
begin
  ...
  if p_new_path is not null and current_path is not distinct from p_new_path then
...
as $$
  select q.storage_path from public.avatar_cleanup_queue q where q.user_id=(select auth.uid())
  union
  select o.name from storage.objects o
  join public.profiles p on p.user_id=(select auth.uid())
  where o.bucket_id='avatars'
    and (storage.foldername(o.name))[1]=(select auth.uid())::text
    and o.name is distinct from p.avatar_path
