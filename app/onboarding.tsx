...
export default function OnboardingScreen() {
  ...
  return <SafeAreaView className="flex-1 bg-background"><ScrollView contentContainerStyle={{ padding: 24, gap: 20 }}><AppText variant="heading">{developmentCopy.onboardingTitle}</AppText><AppText tone="secondary">{developmentCopy.onboardingIntro}</AppText><AppText accessibilityRole="header" variant="title">{developmentCopy.ageNotice}</AppText><Chip label={developmentCopy.ageConfirmation} selected={ageConfirmed} onPress={() => setAgeConfirmed((value) => !value)} />{current.map((key) => <View key={key} style={{ gap: 8 }}><AppText variant="label">{developmentCopy[key]}</AppText><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>{options[key].map(([value, labelKey]) => <Chip key={value} label={developmentCopy.options[labelKey]} selected={prefs[key] === value} onPress={() => set(key, value)} />)}</View></View>)}{error ? <AppText tone="danger" accessibilityLiveRegion="polite">{error}</AppText> : null}<View style={{ gap: 8 }}><Button disabled={!ageConfirmed} loading={saving} onPress={() => void submit()}>{step === groups.length - 1 ? developmentCopy.saveAndExplore : developmentCopy.continue}</Button>{step > 0 ? <Button variant="secondary" onPress={() => setStep(step - 1)}>{developmentCopy.back}</Button> : null}</View></ScrollView></SafeAreaView>;
}
